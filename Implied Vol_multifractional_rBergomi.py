# -*- coding: utf-8 -*-
"""
Created on Mon Jul  6 10:07:35 2026
...
def price_rbergomi_sinusoidal_h(K, T, alpha0, alpha1, eta, rho, xi, S0, r, q, n_steps, n_paths, rng):
    """
    Prices an option using the slow but theoretically consistent 'true memory' rBergomi model.
    This version correctly handles a time-varying Hurst parameter H(t) for both the
    stochastic process Y_t and its theoretical variance Var(Y_t) for the drift correction.
    ...
    Args:
        K (float): Strike price.
        T (float): Time to maturity.
        alpha0 (float): parameter of the polynomial (degree 0)
        alpha1 (float): parameter of the polynomial (degree 1)
        eta (float): Volatility of volatility parameter.
        rho (float): Correlation between the two Brownian motions.
        xi (float): Initial forward variance (V_0).
        S0 (float): Initial stock price.
    ...
    def H_func(t, alpha0=0.25, alpha1=0.4):
        """Returns the Hurst parameter based on the time t."""
        x = alpha0 + alpha1 * np.sin((2 * np.pi * t) / T) 
        return np.clip (x, 1e-4, 0.4999) #the clip prevents H(t) from being outside the interval (0, 1/2)
...
def price_rbergomi_logistic_h(K, T, H_min, H_max, gamma, t_star, eta, rho, xi, S0, r, q, n_steps, n_paths, rng):
    """
    Prices an option using the slow but theoretically consistent 'true memory' rBergomi model.
    This version correctly handles a time-varying Hurst parameter H(t) for both the
    stochastic process Y_t and its theoretical variance Var(Y_t) for the drift correction.
    ...
    def H_func(t):
        ...
        """
        x = H_min + (H_max - H_min) / (1 + np.exp(-gamma * (t - t_star)))
        return np.clip(x, 1e-4, 0.4999) #the clip prevents H(t) from being outside the interval (0, 1/2)
...
def simulate_rbergomi_sinusoidal_h_path(T, alpha0, alpha1, eta, rho, xi, S0, n_steps):
    """
    Simulates a single Variance (V_t) and Price (S_t) path for the rBergomi model
    with the Hurst parameter being a linear function.
...
def simulate_rbergomi_logistic_h_path(T, H_min, H_max, gamma, t_star, eta, rho, xi, S0, n_steps):
    """
    Simulates a single Variance (V_t) and Price (S_t) path for the rBergomi model
    with a piecewise-constant Hurst parameter (regime-switching).
    ...
    def H_func(t):
        ...
        Smoothly transitions from H_min (for t << t_star) 
        to H_max (for t >> t_star).
        """
        x = H_min + (H_max - H_min) / (1 + np.exp(-gamma * (t - t_star)))
...
beta0_main, beta1_main = 0.15, 0.45  # H(t) = 0.15 + 0.45t
alpha0_main, alpha1_main = 0.25, 0.4 # H(t) = 0.25 + 0.4*sin(2*pi*t / 0.5)
H_max_main, H_min_main, gamma_main, t_star_main = 0.05, 0.45, 20, 0.2 # H(t) = 0.05 + 0.4 / (1 + exp(-20*(t-0.2)))
n_steps, n_paths = 100, 300000
moneyness_k_f = np.linspace(0.6, 1.3, 15)
strikes = S0_main * moneyness_k_f
ivols_h1, ivols_h2, ivols_linear, ivols_sinusoidal, ivols_logistic = [], [], [], [], []
...
rng_sinusoidal = np.random.default_rng(seed=3)
rng_logistic = np.random.default_rng(seed=4)
...
# --- Model 4: Sinusoidal H(t): H(t) = 0.25 + 0.4*sin(2*pi*t / 0.5) ---
print(f"\n--- Model 4/5: Sinusoidal H(t): H(t) = 0.25 + 0.4*sin(2*pi*t / 0.5) ---")
for K in strikes:
    flag = 'c' if K >= S0_main else 'p'
    # Here we use H1_main and H2_main to simulate the regime switch.
    price = price_rbergomi_sinusoidal_h(K, T_main, alpha0_main, alpha1_main, eta_main, rho_main, xi_main, S0_main, r_main, q_main, n_steps, n_paths, rng=rng_sinusoidal)
    ivols_sinusoidal.append(find_implied_vol(price, S0_main, K, T_main, r_main, q_main, flag))
    
# --- Model 5: Logistic H(t): H(t) = 0.05 + 0.4 / (1 + exp(-20*(t-0.2))) ---
print(f"\n--- Model 5/5: Logistic H(t): H(t) = 0.05 + 0.4 / (1 + exp(-20*(t-0.2))) ---")
for K in strikes:
    flag = 'c' if K >= S0_main else 'p'
    # Here we use H1_main and H2_main to simulate the regime switch.
    price = price_rbergomi_logistic_h(K, T_main, H_min_main, H_max_main, gamma_main, t_star_main, eta_main, rho_main, xi_main, S0_main, r_main, q_main, n_steps, n_paths, rng=rng_logistic)
    ivols_logistic.append(find_implied_vol(price, S0_main, K, T_main, r_main, q_main, flag))    
...
plot_with_nan_handling(moneyness_k_f, ivols_sinusoidal, marker='v', linestyle=':', color='red', lw=2.5, markersize=8, label=f'Sinusoidal H(t) = 0.25 + 0.4*sin(2*pi*t / 0.5)')
plot_with_nan_handling(moneyness_k_f, ivols_logistic, marker='D', linestyle='-', color='purple', lw=2.5, markersize=8, label=f'Logistic H(t) = 0.05 + 0.4 / (1 + exp(-20*(t-0.2)))')

plt.title(f'Rough Bergomi Implied Volatility Smile: Constant vs. Linear vs. Sinusoidal vs. Logistic Hurst Parameter (T={T_main})\n$\\xi={xi_main}$, $\\eta={eta_main}$, $\\rho={rho_main}$', fontsize=16)
...
# ==============================================================================
# MAIN SIMULATION AND PLOTTING T = 1
# ==============================================================================
# --- Model Parameters ---
S0_main, r_main, q_main = 1.0, 0.0, 0.0
...
# --- Model 4: Sinusoidal H(t): H(t) = 0.25 + 0.4*sin(2*pi*t / 1.0) ---
print(f"\n--- Model 4/5: Sinusoidal H(t): H(t) = 0.25 + 0.4*sin(2*pi*t / 1.0) ---")
for K in strikes:
    flag = 'c' if K >= S0_main else 'p'
    # Here we use H1_main and H2_main to simulate the regime switch.
    price = price_rbergomi_sinusoidal_h(K, T_main, alpha0_main, alpha1_main, eta_main, rho_main, xi_main, S0_main, r_main, q_main, n_steps, n_paths, rng=rng_sinusoidal)
...
#plot_with_nan_handling(moneyness_k_f, ivols_h2, marker='s', linestyle='--', color='orangered', label=f'Constant $H = {H2_main}$')
plot_with_nan_handling(moneyness_k_f, ivols_linear, marker='^', linestyle='-.', color='darkgreen', lw=2.5, markersize=8, label=f'Linear H(t) = 0.15 + 0.45t')
plot_with_nan_handling(moneyness_k_f, ivols_sinusoidal, marker='v', linestyle=':', color='red', lw=2.5, markersize=8, label=f'Sinusoidal H(t) = 0.25 + 0.4*sin(2*pi*t / 1.0)')
plot_with_nan_handling(moneyness_k_f, ivols_logistic, marker='D', linestyle='-', color='purple', lw=2.5, markersize=8, label=f'Logistic H(t) = 0.05 + 0.4 / (1 + exp(-20*(t-0.2)))')
...
# --- Model 4: Sinusoidal H(t): H(t) = 0.25 + 0.4*sin(2*pi*t / 1.5) ---
print(f"\n--- Model 4/5: Sinusoidal H(t): H(t) = 0.25 + 0.4*sin(2*pi*t / 1.5) ---")
for K in strikes:
    flag = 'c' if K >= S0_main else 'p'
    # Here we use H1_main and H2_main to simulate the regime switch.
    price = price_rbergomi_sinusoidal_h(K, T_main, alpha0_main, alpha1_main, eta_main, rho_main, xi_main, S0_main, r_main, q_main, n_steps, n_paths, rng=rng_sinusoidal)
...
#plot_with_nan_handling(moneyness_k_f, ivols_h2, marker='s', linestyle='--', color='orangered', label=f'Constant $H = {H2_main}$')
plot_with_nan_handling(moneyness_k_f, ivols_linear, marker='^', linestyle='-.', color='darkgreen', lw=2.5, markersize=8, label=f'Linear H(t) = 0.15 + 0.45t')
plot_with_nan_handling(moneyness_k_f, ivols_sinusoidal, marker='v', linestyle=':', color='red', lw=2.5, markersize=8, label=f'Sinusoidal H(t) = 0.25 + 0.4*sin(2*pi*t / 1.5)')
plot_with_nan_handling(moneyness_k_f, ivols_logistic, marker='D', linestyle='-', color='purple', lw=2.5, markersize=8, label=f'Logistic H(t) = 0.05 + 0.4 / (1 + exp(-20*(t-0.2)))')
...
# --- Model 4: Sinusoidal H(t): H(t) = 0.25 + 0.4*sin(2*pi*t / 0.5) ---
print(f"\n--- Model 4/5: Sinusoidal H(t): H(t) = 0.25 + 0.4*sin(2*pi*t / 2.0) ---")
for K in strikes:
    flag = 'c' if K >= S0_main else 'p'
    # Here we use H1_main and H2_main to simulate the regime switch.
    price = price_rbergomi_sinusoidal_h(K, T_main, alpha0_main, alpha1_main, eta_main, rho_main, xi_main, S0_main, r_main, q_main, n_steps, n_paths, rng=rng_sinusoidal)
...
#plot_with_nan_handling(moneyness_k_f, ivols_h2, marker='s', linestyle='--', color='orangered', label=f'Constant $H = {H2_main}$')
plot_with_nan_handling(moneyness_k_f, ivols_linear, marker='^', linestyle='-.', color='darkgreen', lw=2.5, markersize=8, label=f'Linear H(t) = 0.15 + 0.45t')
plot_with_nan_handling(moneyness_k_f, ivols_sinusoidal, marker='v', linestyle=':', color='red', lw=2.5, markersize=8, label=f'Sinusoidal H(t) = 0.25 + 0.4*sin(2*pi*t / 2.0)')
plot_with_nan_handling(moneyness_k_f, ivols_logistic, marker='D', linestyle='-', color='purple', lw=2.5, markersize=8, label=f'Logistic H(t) = 0.05 + 0.4 / (1 + exp(-20*(t-0.2)))')
...
t, S, V = simulate_rbergomi_linear_h_path(**params)
...
fig.suptitle(r'Linear Hurst Parameter Rough Bergomi Model - Single Path Simulation', fontsize=18)
...
ax2.set_title(rf'Variance Path ($V_t$) with $H(t)={params["beta0"]} + {params["beta1"]} * t$')
